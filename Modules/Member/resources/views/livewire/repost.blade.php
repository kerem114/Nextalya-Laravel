...
<div class="col-md-6 mb-3">
    <div class="product-post-card">
        ...
        <hr>
         <div class="product-post-header">
            <a href="{{ route('user.account',$take->product->user->username) }}" class="product-post-profile-link">
                <img src="{{ asset('storage/'.$take->product->user->avatar) }}" alt="Profile"
                    class="product-post-profile" />
            </a>
            <div class="product-post-user">
                <a href="{{ route('user.account',$take->product->user->username) }}" class="product-post-name">{{ $take->product->user->name }}</a>
                <a href="{{ route('user.account',$take->product->user->username) }}" class="product-post-username">{{ '@' . $take->product->user->username }}</a>
            </div>
            <div class="product-post-actions ">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#collection-{{ $take->product->id }}">Koleksiyona kaydet</a></li>
                </ul>
            </div>
        </div>
        <div class="product-post-title-row">
            ...
            @if($take->product->url)
            <a href="{{ $take->product->url }}" target="_blank">
                <i class="bi bi-box-arrow-up-right"></i>
            </a>
            @endif
        </div>
        @if(Str::startsWith($take->product->image, 'http'))
            <img class="product-post-image" src="{{ $take->product->image }}" class="search-product-image" />
        @else
            <img class="product-post-image" src="{{ asset('storage/product/' . $take->product->image) }}" class="search-product-image" />
        @endif
        ...
        <div class="product-post-footer">
            <div class="product-post-left-icons">
                <a href="#" style="margin-top: 5px" class="product-link-post-icon" data-product-id="{{ $take->product->id }}">
                ...
                <a href="{{ route('product.detail',$take->product->slug) }}" class="product-post-icon">
                    <i class="bi bi-chat"></i>
                    <span>4</span>
                </a>
                <a href="#" data-bs-toggle="modal" data-bs-target="#repost-{{ $take->product->id }}" class="product-post-icon">
                    <i class="bi bi-arrow-repeat"></i>
                </a>
            </div>
            <div class="product-post-right-icons">
            @if(\Modules\Save\Models\Favories::isSave($take->product->id))
                    <a style="margin-top: 5px" wire:navigate href="{{ route('product.favories', $take->product->id) }}" class="product-link-post-iconx"><i class="bi bi-bookmark-fill"></i></a>
                @else
                    <a style="margin-top: 5px" wire:navigate href="{{ route('product.favories', $take->product->id) }}" class="product-link-post-iconx"><i class="bi bi-bookmark"></i></a>
                @endif
                <a href="#" data-bs-toggle="modal" data-bs-target="#shareModal{{ $take->product->id }}" class="product-post-icon">
    ...
    <div class="text-center">
        ...
        <p class="mt-2"><b>Henüz hiç repost yok</b></p>
    </div>
    @endforelse
@if($perPage < $total)
